...
| Organization / Tenancy | `src/modules/organization`; HTTP routes still use `/tenant-owners` for compatibility | Tenant ownership, tenant membership, platform/tenant scopes | Depends on Identity for actor identity; legacy `tenant-owners` module folder should not be reintroduced. |
...
| Guest Operations | `src/modules/guest-operations`; HTTP routes still use `/guest` and `/hotels/:hotelId/requests*` for compatibility | Guest sessions, guest requests, request timeline/status, staff request workflow | Public guest session/service ports are exported via `guest-operations-public.ts`; staff workflow publishes request events through the shared event port. |
| Billing | `src/modules/billing` (`api`, `application`, `domain`, `infrastructure`, `tests`) | Folios, folio items, invoices, payments, checkout rules | Uses property access, should not depend on property persistence internals. |
| Emergency | `src/modules/emergency` (`api`, `application`, `domain`, `infrastructure`, `tests`); HTTP route still uses `/emergency/guest/calls` for compatibility | Emergency locations, incidents, call lifecycle, notifications | Consumes guest-session context through the Guest Operations public port; repository internals are private. |
