...
export async function POST(_request: Request, context: Params) {
  ...
  try {
    const data = await executeOwnerBackendRequest("rotate owner room qr", (accessToken) => hotelOpsService.rotateRoomQr(hotelId, roomId, accessToken));
    if (data instanceof Response) return data;
    return successResponse(data, 200, "Owner room QR rotated successfully");
  } catch (error) {
