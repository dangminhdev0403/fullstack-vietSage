...
export async function POST(_request: Request, context: Params) {
  ...
  try {
    const data = await executeOwnerBackendRequest("activate owner room qr", (accessToken) => hotelOpsService.setRoomQrActive(hotelId, roomId, true, accessToken));
    if (data instanceof Response) return data;
    return successResponse(data, 200, "Owner room QR activated successfully");
  } catch (error) {
