...
## [in review — visual gate blocked] 2026-09-06 — Mission: isolated-admin-ui-overhaul

### Isolation and evidence

- Worktree `admin-ui-overhaul` is independent: Orca parent/lineage null, base `origin/master`.
  `git ls-remote --symref origin HEAD` confirmed repository default branch `master`; remote HEAD,
  `origin/master` and starting HEAD all matched `9450196b8fe32fe0fe1f9682b5b539a92c3dfb3d`.
  No reads/edits were taken from `cccd-qr-mobile-relay`. No commits, pushes, merges or deployment.
- Read root/canonical/frontend AGENTS, frontend architecture/rules/runtime/module guides, Design DNA
  (`docs/DESIGN.md`), UI Quality Pro-Max and supporting Impeccable operational guidance.
  Requested `PONYTAIL.md` and frontend `.agents/AGENTS.md` are absent from this checkout and checked ancestors.
  Existing frontend `AGENTS.md` applies. No replacement policy files created.
- Navigation: Graphify queries/affected maps for shell, navigation, tables and top bar; bounded
  `admin-shell.xml`, `admin-routes.xml`, `admin-surfaces.xml` and `admin-checks.xml` Repomix packs;
  exact source then inspected. No scanner exclusions. Graphify does not index the CSS/design-policy
  paths; bounded tracked frontend inventory resolved that gap. One overlong Repomix command was
  replaced with the same bounded scope using grouped path patterns.
- Graphify package 0.9.20 reports installed skill version 0.9.33 mismatch; navigation still works.
  Impeccable's guest-only PRODUCT.md context is not an administration specification; left untouched.

### What changed / surface audit

- Shared shell: light navy/gold VietSage tokens, 72px header, 288px sidebar / 88px collapsed rail,
  16px navigation labels with 24px line height, 48px rows, 24px icons, visible active/focus states,
  full labels rather than truncation. Drawer below 1024px replaces the inaccessible four-link bottom bar.
- Full authorized navigation uses one renderer on desktop/mobile. Native dialog provides modal focus,
  Escape/backdrop/route-change closure; resizing to desktop closes it. Skip link reaches the main landmark.
- Existing sidebar preference key retained through installed Zustand, deferred hydration, safe handling
  of denied storage. No navigation/capability registry, route, auth or API behavior altered.
- Platform, owner and reception dashboards use clear page headers, responsive operational metrics,
  restrained panel grouping and readable action hierarchy rather than oversized decorative hero cards.
- Tables/filters/forms: readable text and controls throughout the shell, keyboard-scrollable raw tables,
  named searches, larger pagination, sort/selection semantics, tabular numerals. Shared empty/error
  recovery sits outside the wide scroller; nested control key events no longer activate the row.
- Content loading/error states get stable spacing and readable recovery. Account password dialog uses
  native modal focus/cancellation while retaining pending protection, validation and password visibility.
- Page headers cover hotel portfolio/details, users, marketplace, queues, services, rooms and billing;
  staff assignment state is separately improved because it intentionally has no hotel shell.
- Permissions/roles, marketplace editor, staff-management, service catalogues, room grids, folios,
  SaaS billing, partners and biometric workspaces inherit shared type, controls, focus and responsive rules.
  Messages retain their existing composer/viewport layout. Invoice and QR export content/printing retained.
  Device/check-in implementation is not modified; no camera/device or browser actions performed.
- Vietnamese operational copy retained; existing English/Russian/category/guest locale data untouched.
  Regression checks exercise complete Vietnamese, English and Cyrillic navigation labels. No unsupported
  claim that the baseline Vietnamese administration itself already has a complete locale switcher.

### SSR / client decisions — every route

Source audit: **44 pages, 8 layouts, 11 loading/error boundaries**. All **39 existing server page
boundaries retained**. Five existing service-portal client entries retained; zero pages converted to client.
Server Components passed as shell children remain server-rendered: a client shell does not import the pages.

| Route | Decision / reason |
| --- | --- |
| `/admin` | Server redirect to existing dashboard. |
| `/admin/billing` | Server route composition; existing billing client owns queries/forms; admin layout enforces access. |
| `/admin/dashboard` | Server persona/capability projection and legacy tab redirects; no browser state needed. |
| `/admin/hotels` | Server auth and parallel initial hotels/tenant selectors; client manages filters/editing. |
| `/admin/marketplace` | Server composition; marketplace client owns interactive catalogue; admin guard retained. |
| `/admin/permissions` | Server role/permission loading, URL selection and partial-failure handling; browser/workbench stays leaf-client. |
| `/admin/roles` | Server auth, URL input and role loading; live filter remains interactive leaf. |
| `/admin/users` | Server tenant/owner initial data and capability-filtered tabs; user/staff editors stay client. |
| `/hotels/[hotelId]/billing` | Server hotel/capability checks and parallel folio/dashboard/analytics loading; checkout leaf remains client. |
| `/hotels/[hotelId]/billing/invoices/[invoiceId]` | Server invoice fetch and permission checks; print/action leaves retained. |
| `/hotels/[hotelId]/biometric` | Server hotel/stay capability guard; device tabs remain existing client leaves. No device access performed. |
| `/hotels/[hotelId]/dashboard` | Server dashboard data and capability checks; workstation panel remains existing leaf. |
| `/hotels/[hotelId]/messages` | Server hotel/message guard; realtime room conversation client unchanged. |
| `/hotels/[hotelId]/partners` | Server hotel/capability guard; partner operations remain leaf-client. |
| `/hotels/[hotelId]/requests` | Server URL filters, capabilities and parallel queue/summary/service initial data; queue leaf unchanged. |
| `/hotels/[hotelId]/requests/[requestId]` | Server redirect retains existing queue/detail selection URL. |
| `/hotels/[hotelId]/rooms` | Server hotel/stay checks and initial rooms/arrivals/dashboard; reception/check-in leaves unchanged. |
| `/hotels/[hotelId]/services` | Server capability checks and initial categories/items; catalogue editing remains leaf-client. |
| `/owner` | Server redirect to existing hotel portfolio. |
| `/owner/dashboard` | Server session, hotel/dashboard/revenue aggregation; existing animated-number leaf retained. |
| `/owner/hotels` | Server persona/capability check; existing portfolio query client remains client. |
| `/owner/hotels/[hotelId]` | Server scoped hotel lookup/authorization; details client remains interactive leaf. |
| `/owner/hotels/[hotelId]/billing` | Server folio initial loading and search parameters; folio/SaaS/tab clients remain interactive leaves. |
| `/owner/hotels/[hotelId]/billing/invoices/[invoiceId]` | Server authorized invoice data; print button stays client. |
| `/owner/hotels/[hotelId]/biometric` | Server parameter composition under owner/hotel guards; existing device client retained. |
| `/owner/hotels/[hotelId]/partners` | Server workspace capability projection; partner client remains leaf. |
| `/owner/hotels/[hotelId]/requests` | Server parallel queue/summary/services loading and URL filters; shared queue client retained. |
| `/owner/hotels/[hotelId]/requests/[requestId]` | Server redirect retains request selection URL. |
| `/owner/hotels/[hotelId]/rooms` | Server rooms initial data and error fallback; room actions remain client. |
| `/owner/hotels/[hotelId]/rooms/qr-export` | Server hotel parameter composition; QR export/printing remains existing leaf-client. |
| `/owner/hotels/[hotelId]/services` | Server hotel/category/item initial data; catalogue client retained. |
| `/owner/hotels/[hotelId]/stay` | Server redirect to canonical rooms route; retained legacy grid component still benefits from shared controls. |
| `/owner/staff` | Server tenant selection/capability checks and hotel selectors; staff management stays client. |
| `/service` | Server redirect to partner dashboard. |
| `/service/catalog` | Existing client route uses service-portal hooks; server service-layout auth retained. No conversion. |
| `/service/dashboard` | Existing client route uses service-portal hooks; server service-layout auth retained. No conversion. |
| `/service/orders` | Existing client route uses service-portal hooks; server service-layout auth retained. No conversion. |
| `/service/settings` | Existing client route owns service-portal editing; server service-layout auth retained. No conversion. |
| `/service/settlements` | Existing client route uses service-portal hooks; server service-layout auth retained. No conversion. |
| `/staff` | Server assignment/persona decision and hotel redirect; unassigned state remains SSR. |
| `/staff/dashboard` | Server alias redirect to /staff retained. |
| `/staff/front-desk` | Server alias redirect to /staff retained. |
| `/staff/manager` | Server alias redirect to /staff retained. |
| `/staff/operations` | Server alias redirect to /staff retained. |

All eight layouts remain server boundaries: `admin/layout` and `staff/layout` retain session/role/refresh
checks; `hotels/layout` retains its outer guard; `hotels/[hotelId]/layout` retains hotel/capability scope;
`owner/layout` retains owner authentication, global/hotel owner layouts retain scoped navigation;
`service/layout` retains partner-role auth. All eleven loading/error files retain their existing
streaming/error-boundary ownership. Shared content-state presentation adds no client directive.

Client decisions: WorkspaceShell still owns pathname/search state, message unread subscriptions and
UI events; the new preference store is consumed only there. Sidebar navigation is an existing client
leaf, reused inside the drawer. Top-bar server compatibility retained via optional ReactNode slot.
Password dialog remains an interactive client leaf. DataTable remains directive-free, preserving
existing server-compatible presentation and existing interactive consumers. No new server-state layer.

### Validation

- Test-first: four initial shell/table/store checks failed against baseline; SSR preservation passed.
  Added failing row-key/empty-recovery and native-dialog checks before their respective fixes.
- `node --test scripts/admin-ui-regression.test.mjs scripts/chat-layout-regression.test.mjs scripts/password-ui-entrypoints.test.mjs scripts/tenant-room-action-icons.test.mjs scripts/staff-room-qr-regression.test.mjs src/features/workspace/utils/workspace-nav-active.test.ts src/features/workspace/config/workspace-registry.test.ts`: **36/36 pass**.
- `node node_modules/eslint/bin/eslint.js <changed TS/TSX/MJS paths> scripts/admin-ui-regression.test.mjs src/features/workspace/store/workspace-sidebar-store.ts`: **pass**. Baseline administration/shared-surface ESLint also passed.
- `node node_modules/typescript/bin/tsc --noEmit --incremental false`: **pass**, baseline and changed tree.
- Scoped TypeScript compiler API `createProgram` using tsconfig options, changed TS/TSX entrypoints plus
  ambient declarations and new store: **40 roots, zero diagnostics**, no emit/incremental output.
- Bare production build baseline: blocked by missing/invalid production `AUTH_SECRET` / `NEXTAUTH_SECRET`
  while collecting `/api/auth/[...nextauth]`. Build with a randomly generated, process-only `AUTH_SECRET`
  passed; no secret saved or printed. Final post-source-edit rebuild: **pass**, compiled in 26.1s,
  TypeScript in 19.5s, 56/56 generation steps completed; administration routes remain dynamic.
- Installed existing dependencies only: pnpm 11 frozen install rejected legacy overrides; `npx --yes pnpm@10 install --frozen-lockfile --ignore-scripts` succeeded without package/lockfile changes.
- `git diff --check`: pass using repository line-ending settings. `git diff --exit-code -- '**/package.json' '**/*lock*' services shared tools`: pass. No backend/database/contracts/dependency edits.
- Impeccable detector over changed targets: exit 2, **57 advisories / 8 warnings** (not a visual pass).
  Warnings point to retained legacy palette/print/side-border styles. New CSS findings include selectors
  that raise legacy 10/11px text to 14px (false positives) and the documented 36px administration heading.
- Static viewport/zoom contract covers 320, 360, 390, 430, 768, 820, 844 landscape-width, 1024, 1280,
  1440, 1920 and 125/150/200% width calculations. This is **not** rendered responsive verification.
- `graphify update . --force`: **pass**, AST-only/no LLM, 8,201 nodes / 18,771 edges.
  Existing tooling gaps: 11 non-code/metadata files produce zero nodes, SQL grammar missing for 78 SQL
  files; neither blocks the frontend code graph. Graph HTML visualization was correctly skipped at size limit.
- Final bounded Repomix refresh: **4/4 pass**, security scanner reports no exclusions. Shell: 23 files /
  16,916 tokens; routes: 63 / 9,590; surfaces: 57 / 22,778; checks: 7 / 1,004. Packs inspected after refresh.
  The shell pack intentionally omits this large historical PLANS file; this mission was read/edited by exact range.

### Acceptance blockers / residual risks

- **Browser/visual gate BLOCKED by explicit user instruction**. No browser, E2E, screenshot, camera or
  device run. Real font metrics, touch, modal focus restoration, intermediate-width reflow, print layout,
  live auth/data/realtime/error flows still require separately authorized verification before release.
- Production deployment still requires a real valid auth secret; the temporary build value is not a
  deployment configuration and was discarded after each build command.
- Existing custom workflow dialogs, service-portal client routes and page-specific legacy styles are
  deliberately retained. Shared control/type rules can reveal dense-layout pressure with real long data.
- An environment helper created untracked `%SystemDrive%/ProgramData/Microsoft/Windows/Caches/` database
  artifacts inside this worktree. Verified in-workspace cleanup was rejected by tool policy; preserve/exclude
  those artifacts from the later merge and remove them manually after reviewing the literal path.
- Orca comments/status updated at discovery, shell implementation and verification checkpoints;
  final status intended as **in-review**, not deployed/visually accepted.

### Exact frontend files changed

Paths below are relative to `frontends/front-end-vietsage/`. Generated graph/pack files are separate
review artifacts, not product source. Unrelated code and all original routes remain intact.

- `docs/DESIGN.md`
- `docs/PLANS.md`
- `docs/RULES.md`
- `scripts/admin-ui-regression.test.mjs`
- `src/app/(vietsage)/_components/route-boundary-state.tsx`
- `src/app/(vietsage)/_components/vs-dashboard-sidebar.tsx`
- `src/app/(vietsage)/_components/vs-top-bar.tsx`
- `src/app/(vietsage)/admin/billing/admin-billing-client.tsx`
- `src/app/(vietsage)/admin/dashboard/page.tsx`
- `src/app/(vietsage)/admin/hotels/hotels-admin-client.tsx`
- `src/app/(vietsage)/admin/hotels/page.tsx`
- `src/app/(vietsage)/admin/marketplace/page.tsx`
- `src/app/(vietsage)/admin/permissions/_components/permission-workbench.tsx`
- `src/app/(vietsage)/admin/roles/_components/roles-live-filter.tsx`
- `src/app/(vietsage)/admin/users/page.tsx`
- `src/app/(vietsage)/admin/users/tenant-owners-client.tsx`
...
- `src/app/(vietsage)/owner/(global)/hotels/owner-hotels-client.tsx`
...
- `src/components/ui/data-table/data-table-states.tsx`
- `src/components/ui/data-table/data-table.tsx`
- `src/features/account/security/change-password-dialog.tsx`
- `src/features/marketplace-admin/marketplace-admin-client.tsx`
- `src/features/workspace/components/workspace-shell.tsx`
- `src/features/workspace/components/workspace.css`
- `src/features/workspace/store/workspace-sidebar-store.ts`

### Exact generated / tooling artifact inventory

These paths are relative to the repository root. Graphify preserved its five-file previous graph
backup automatically. The four `%SystemDrive%` paths are unintended environment caches, not merge scope.
...
- `%SystemDrive%/ProgramData/Microsoft/Windows/Caches/{A125BC7A-62B1-4F82-971B-12F2C91C819A}.2.ver0x0000000000000001.db`
- `%SystemDrive%/ProgramData/Microsoft/Windows/Caches/{DDF571F2-BE98-426D-8288-1A9A39C3FDA2}.2.ver0x0000000000000001.db`
- `graphify-out/2026-09-06/.graphify_analysis.json`
- `graphify-out/2026-09-06/.graphify_labels.json`
- `graphify-out/2026-09-06/GRAPH_REPORT.md`
- `graphify-out/2026-09-06/graph.json`
- `graphify-out/2026-09-06/manifest.json`
- `graphify-out/cache/ast/v0.9.20/01ea65ff630232160bbba85de5bbb0903950aec753b82a0c92d7c801ce334b37.json`
- `graphify-out/cache/ast/v0.9.20/052c93c68733bc9bd210697b334d5b581316b06edf6adaa19cd8db657f3486b9.json`
- `graphify-out/cache/ast/v0.9.20/05c9d1fc4ac423022041935334e6627bdeaa0ed13454b69dd18c674efbdf4bf7.json`
- `graphify-out/cache/ast/v0.9.20/07ce2d4cdec89474ed4678032ac0cb1bc11f0b0d33854ce7794f8c449ab59671.json`
- `graphify-out/cache/ast/v0.9.20/10f0773377b9f37c852fcb043eb581c2e69ee2b9ccaacb1593a2da61f273e3c3.json`
- `graphify-out/cache/ast/v0.9.20/174f2546ceef11e9a9733b0a08d50bba08b00a54c3a2c8f3d3d3bf6de89ad322.json`
- `graphify-out/cache/ast/v0.9.20/1ad40b6fa00a4a1d63937c0cca521b9d717bdbdb2c53011e612636b7b8226819.json`
- `graphify-out/cache/ast/v0.9.20/1cdf36d2e247d2868deb92547abec352f15b16d3e6083ee2bdde0810f0ad81c7.json`
- `graphify-out/cache/ast/v0.9.20/20ac8c57467d41d08980d50be2300c45e7431d004e28e18374f2b3217396da19.json`
- `graphify-out/cache/ast/v0.9.20/23187b5b54dd2c0c8446f8b77b88593fc184f44fa08e039c49a5ce6630cda946.json`
- `graphify-out/cache/ast/v0.9.20/29ef2eccd95e406e5b20786f63146eb6d2b9f96913000c46dc6620c419fb1daa.json`
- `graphify-out/cache/ast/v0.9.20/2cd9cd96e128f8a15b4cabd337b40756a05ea9d019adedf3928a0e9c33a36b5b.json`
- `graphify-out/cache/ast/v0.9.20/337b8e49bf1b450e7ff5a244880d8512ad8f5aade8d181026e99c4eee49f1d32.json`
- `graphify-out/cache/ast/v0.9.20/367da3eaba292e4dd4442046a59baa61ba2f6049729f829721da8a07d7646547.json`
- `graphify-out/cache/ast/v0.9.20/3a120215a25cb2c7ebcb496bf46692cd9ac9ff5a6624f251dc373ad6432fb435.json`
- `graphify-out/cache/ast/v0.9.20/3b51241a6218165f17341e7d8b7277977c3bf377d0f3f838e34af29a7e8ca77b.json`
- `graphify-out/cache/ast/v0.9.20/3ca002171360e05ede4f8a5963ab8617cab0380d0b3130fa23f58a61b92d2dce.json`
- `graphify-out/cache/ast/v0.9.20/3cfdc6af8971249911a4065718d035bc1e39e50c5bae2ce31811a302b1d0727d.json`
- `graphify-out/cache/ast/v0.9.20/40247a3b881f9f0a7f0789c9ea900f30a811c6755a0d74b331a987755e0e8408.json`
- `graphify-out/cache/ast/v0.9.20/42de5702d9ca5920d49b2cf3ba68d9ab4b88771df3b0db1c7718c53acf659581.json`
- `graphify-out/cache/ast/v0.9.20/46a44b3e6e1f77e8ea4a1ac31a24a28ab369fb95a9ea4993b5a15ec188cb83b8.json`
- `graphify-out/cache/ast/v0.9.20/4c3f770e3a59b4ba46e0ce258208b308a11b9f8f63b238ab61e295960deda8e3.json`
- `graphify-out/cache/ast/v0.9.20/5ea1ca2973c536ff5e2b0ba179b3e43a1a7e753f7c3cf4064669e041721e3857.json`
- `graphify-out/cache/ast/v0.9.20/6216b70906ff0d0b24c801e9546e39759c4d71bf6d2df48b625382e0f882cc1f.json`
- `graphify-out/cache/ast/v0.9.20/66c91007ff7374df676e5840ba19015216648da8f7016df56f7ff29e904210f4.json`
- `graphify-out/cache/ast/v0.9.20/6dd4230c9e73a1a881e27b8a78e94d59377c5464c6a1167d42df638b866c6ffa.json`
- `graphify-out/cache/ast/v0.9.20/6dfdfe2f655eeeb81dc5c1d9114da4d15789b69dbc40fda7ec7f398b12b3325c.json`
- `graphify-out/cache/ast/v0.9.20/6f54e89822a684addd90a5d4fea14f058b1122e954ab558f0cc847121ab44147.json`
- `graphify-out/cache/ast/v0.9.20/72a431bcce2f8755f3127a0f59d4225536f1516d7754ef7f65f6ca783712eb01.json`
- `graphify-out/cache/ast/v0.9.20/745912c0bbbf8c31a2d15d88b8c93f771b7603de0cd66f4dead7c2f601d68bc7.json`
- `graphify-out/cache/ast/v0.9.20/81eff57b1f6fc8b4a6bd17bba97bd94c9552103b9d653454b595f2eaac565760.json`
- `graphify-out/cache/ast/v0.9.20/842b75c346ff46d183f60968700df7d67808f7687803701cef3a26135c4dc36d.json`
- `graphify-out/cache/ast/v0.9.20/84780b60f6d2508f849182496d539408debf550d00b27397657978200c1772ac.json`
- `graphify-out/cache/ast/v0.9.20/8506a3667eb95068e30ce5a0a49d885931a6026b7b9a3f9e5d8e693b27301c92.json`
- `graphify-out/cache/ast/v0.9.20/88c34d44a52ba37b40cb97a4f08ba735bae52201c8213bd7bd16d1b7db476fd6.json`
- `graphify-out/cache/ast/v0.9.20/8a5cb4536bcb733afbf360c2a473b594aa5b55f8352921b5fd112f8a6b155e80.json`
- `graphify-out/cache/ast/v0.9.20/8d02fdc46b12dcbf8b4e71749398d6b17ea31c8eff24be1d8b9513dfbee24c2c.json`
- `graphify-out/cache/ast/v0.9.20/8e24096c13e6b4dd00fbd7fdab3d650c17ed3f32d5f37af0ee9969bcbb8f5368.json`
- `graphify-out/cache/ast/v0.9.20/96fa39c0b6254ac38ca195fb0d966757354824d9d948a6203eac935adfc3fc7c.json`
- `graphify-out/cache/ast/v0.9.20/9e20051527afd5f6ad7d9d82973280062298e37ce497f5c8897643a096b5657d.json`
- `graphify-out/cache/ast/v0.9.20/bbcaa558c79bd79cefd003634b2b3761d577a43a7fe0953436ad6ca0cc923592.json`
- `graphify-out/cache/ast/v0.9.20/bef855bfcb98565050e445f526d6daab5f895189bd6c458d2fa57ee2263f6062.json`
- `graphify-out/cache/ast/v0.9.20/c3b4f90aff16578fb6c5300ddb45830fc81bda9d82f6b6de8ff372476e70937a.json`
- `graphify-out/cache/ast/v0.9.20/c43ff0e86f84d886d91c0c6b5cdf377cfa643bfbd6c4626d55c00caf70209197.json`
- `graphify-out/cache/ast/v0.9.20/c60a4970f0753b6026f8462162c1ddc04ccd306dffc8779c9d8f5f3cdd46b4bb.json`
- `graphify-out/cache/ast/v0.9.20/c678a305b54d2c07f512a1c7eca6a91e1713235b324dcef4824672503e3906d6.json`
- `graphify-out/cache/ast/v0.9.20/c941de1aee442d519aa0c6080309071797a59917e85a4199a93f39d5c8bb432f.json`
- `graphify-out/cache/ast/v0.9.20/c9d4c158b5ba55936ea79d51ec26cdfb2fdc4aab9fa3dbdab94c52a1eec53bf6.json`
- `graphify-out/cache/ast/v0.9.20/d10d3283921f693364c950173cfa5cbb519b5eb20617f01e78c8da87375e6cee.json`
- `graphify-out/cache/ast/v0.9.20/d46c96d1e228da68bf517fa06d27ec85289491c7f417d4c3256769d7212b9509.json`
- `graphify-out/cache/ast/v0.9.20/d662f5fd9c1540fffa8908601ae77fa1a4b889dac23683ec43c5fae529ba5f81.json`
- `graphify-out/cache/ast/v0.9.20/d6bcbca531dfcebc537f7e13ab2377224f7a282aed4fc869c037c50b93b7baf0.json`
- `graphify-out/cache/ast/v0.9.20/d720e12aea8fad5cf0fecf69b533ddfd5cab737a0136d306ead8ae93be5f5644.json`
- `graphify-out/cache/ast/v0.9.20/da98fd278da5d4a8a61b0020f5f1077e9abcc42145c00fbe1734ccc3498de104.json`
- `graphify-out/cache/ast/v0.9.20/dad5d24e4780e7b6aa41a0c13188941bfd5c7e9015b7fc7103e9f33a29afc248.json`
- `graphify-out/cache/ast/v0.9.20/dcc8828c6c389f1f0960c010611050a3d01ef2a6e19fac90d9a78a5c5616c0a5.json`
- `graphify-out/cache/ast/v0.9.20/dda57912bdff1ad935ca75e1d274fd0b75310e43f1fe9d44049cb751d7bebb01.json`
- `graphify-out/cache/ast/v0.9.20/de0036391535d34246147bd480d3d001f95458ea586de897ef546155733fc88d.json`
- `graphify-out/cache/ast/v0.9.20/df743b89058b87c21b9649f231870093fa462622fe3a2a7467878e65b38cea1a.json`
- `graphify-out/cache/ast/v0.9.20/e3b04cdd4ba13c88dc8f454d5a25ed978351a02b7551f2b76dd72eb3667a481d.json`
- `graphify-out/cache/ast/v0.9.20/e473e207b439a4eca9a97e4eb8b7f843e5b4f02d6be169169aeecd642f35a492.json`
- `graphify-out/cache/ast/v0.9.20/f93f1fd658403857f5a8c42a76a52cfa43d5c59aff5ee17c1c5111e128ecb4d5.json`
- `graphify-out/cache/ast/v0.9.20/fbe8fe974e77d5ce56a9c1ce1ab84e6cfea658e0fe87231bcd0787a74bdb74e9.json`
- `graphify-out/repomix/admin-checks.xml`
- `graphify-out/repomix/admin-routes.xml`
- `graphify-out/repomix/admin-shell.xml`
- `graphify-out/repomix/admin-surfaces.xml`
