...
| `resources/` | Capability declarations, stable keys, query options, mutations, local invalidation, and cache operations built with `@dangminhdev/query-resource`. |
...
5. Declare only the resource capabilities the backend actually supports.
