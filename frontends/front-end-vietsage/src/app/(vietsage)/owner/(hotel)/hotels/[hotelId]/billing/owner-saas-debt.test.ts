...
test("OwnerSaasBillingClient source contract for debt and projected payment visibility", async (t) => {
  ...
  await t.test(
    "type DTO uses projected debt fields and omits raw settlements",
    () => {
      assert.ok(
        code.includes("settledAmount"),
        "Period DTO must include settledAmount",
      );
      assert.ok(
        code.includes("outstandingAmount"),
        "Period DTO must include outstandingAmount",
      );
      assert.ok(
        code.includes("paymentState"),
        "Period DTO must include paymentState",
      );
      assert.ok(
        code.includes("isOverdue"),
        "Period DTO must include isOverdue",
      );
      assert.equal(
  ...
  await t.test(
    "renders Vietnamese labels: Đã thanh toán, Còn phải trả, Quá hạn",
    () => {
      assert.ok(
        code.includes("Đã thanh toán"),
        "Must render label 'Đã thanh toán'",
      );
      assert.ok(
        code.includes("Còn phải trả"),
        "Must render label 'Còn phải trả'",
      );
  ...
  await t.test(
    "does not infer payment status from p.status === 'PAID' or reduce settlements",
    () => {
      assert.equal(
