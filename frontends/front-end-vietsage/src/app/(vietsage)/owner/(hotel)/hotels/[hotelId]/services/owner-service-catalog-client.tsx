...
const translationLocales = catalogLocales.filter(
  (locale): locale is { key: TranslationLocale; label: string } =>
    locale.key !== "vi",
);

function emptyTranslations(): TranslationFormState {
  return Object.fromEntries(
    translationLocales.map(({ key }) => [key, { name: "", description: "" }]),
...
function formatVnd(
  ...
  if (value === null || value === undefined || value === "") return "--";
  const amount =
    typeof value === "number" ? value : Number(toRawPriceDigits(String(value)));
...
function formatQuantityRule(
  item: Pick<
    HotelServiceItem,
    "quantityEnabled" | "minQuantity" | "maxQuantity"
  >,
): string {
  if (!item.quantityEnabled) return "Không yêu cầu số lượng";
  return item.maxQuantity === null
    ? `SL ${item.minQuantity}+`
    : `SL ${item.minQuantity}-${item.maxQuantity}`;
}
...
function sortIndicator<TKey extends string>(
  sort: SortState<TKey>,
  key: TKey,
): string {
    ...
    <div className="flex flex-col gap-3 border-t border-[var(--outline-variant)] bg-[var(--surface-container-low)] px-4 py-3 text-sm text-[var(--on-surface-variant)] md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={page <= 1}
          className="rounded-lg border border-[var(--outline-variant)] bg-white px-3 py-2 text-sm font-semibold text-[var(--primary)] transition-colors hover:bg-[var(--surface-container-low)] disabled:cursor-not-allowed disabled:opacity-50"
        >
          Trước
        </button>
...
function getItemCategory(
  ...
  categoryById: Map<string, HotelServiceCategory>,
): HotelServiceCategory | null {
...
function getItemEffectivePrice(
  item: HotelServiceItem,
  category: HotelServiceCategory | null,
...
function syncItemsForCategoryUpdate(
  ...
  return items.map((item) => {
    ...
    const priceOverride =
      priceUpdateMode === "OVERRIDE_ALL_ITEMS"
...
export function OwnerServiceCatalogClient({
  ...
  const [itemSort, setItemSort] = useState<SortState<ItemSortKey>>({
    key: "name",
    direction: "asc",
  });
    ...
    () =>
      [...filteredCategories].sort((left, right) => {
        ...
        if (categorySort.key === "name") {
          result = left.name.localeCompare(right.name, "vi", {
            sensitivity: "base",
          });
        } else if (categorySort.key === "price") {
          result =
            toComparablePrice(left.defaultPrice) -
            toComparablePrice(right.defaultPrice);
        } else if (categorySort.key === "sortOrder") {
          result = left.sortOrder - right.sortOrder;
        } else {
          result = serviceStatusLabelMap[left.status].localeCompare(
            serviceStatusLabelMap[right.status],
            "vi",
            { sensitivity: "base" },
          );
        }
    ...
    () =>
      [...filteredItems].sort((left, right) => {
        ...
        if (itemSort.key === "name") {
          ...
          result = (leftCategory?.name ?? left.categoryId).localeCompare(
            rightCategory?.name ?? right.categoryId,
            "vi",
            { sensitivity: "base" },
          );
  ...
  const categoryPageCount = getPageCount(
  ...
  const paginatedCategories = useMemo(
    () => getPageItems(sortedCategories, activeCategoryPage, categoryPageSize),
    [activeCategoryPage, categoryPageSize, sortedCategories],
  );
  const paginatedItems = useMemo(
    () => getPageItems(sortedItems, activeItemPage, itemPageSize),
    [activeItemPage, itemPageSize, sortedItems],
  );
    ...
    () => [
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setCategorySort((current) => toggleSort(current, "name"))
            }
            className={sortableHeaderClass(categorySort.key === "name")}
          >
            Tên{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(categorySort, "name")}
            </span>
          </button>
        ),
        cell: (category) => (
      ...
      },
      {
        ...
        cell: (category) =>
          category.id_group ? (
            <span className="rounded-full bg-[var(--primary-fixed)] px-2.5 py-1 text-xs font-bold text-[var(--on-primary-fixed-variant)]">
              {category.id_group}
            </span>
          ) : (
      ...
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setCategorySort((current) => toggleSort(current, "price"))
            }
            className={sortableHeaderClass(categorySort.key === "price")}
          >
            Giá mặc định{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(categorySort, "price")}
            </span>
          </button>
        ),
        cell: (category) => (
          <span className="font-semibold">
            {formatVnd(category.defaultPrice, category.currency)}
          </span>
        ),
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setCategorySort((current) => toggleSort(current, "sortOrder"))
            }
            className={sortableHeaderClass(categorySort.key === "sortOrder")}
          >
            Thứ tự{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(categorySort, "sortOrder")}
            </span>
          </button>
        ),
      ...
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setCategorySort((current) => toggleSort(current, "status"))
            }
            className={sortableHeaderClass(categorySort.key === "status")}
          >
            Trạng thái{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(categorySort, "status")}
            </span>
          </button>
        ),
        cell: (category) => (
          <span
            className={`rounded-full px-2.5 py-1 text-xs font-bold ${serviceStatusTone(category.status)}`}
          >
      ...
      },
      {
        ...
        cell: (category) => (
          <button
            ...
            className="cursor-pointer rounded-lg px-3 py-2 text-sm font-semibold text-[var(--primary)] transition-colors hover:bg-[var(--surface-container-low)]"
          >
            Sửa
          </button>
        ),
    ...
    () => [
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setItemSort((current) => toggleSort(current, "name"))
            }
            className={sortableHeaderClass(itemSort.key === "name")}
          >
            Tên dịch vụ{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(itemSort, "name")}
            </span>
          </button>
        ),
        cell: (item) => (
      ...
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setItemSort((current) => toggleSort(current, "category"))
            }
            className={sortableHeaderClass(itemSort.key === "category")}
          >
            Nhóm dịch vụ{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(itemSort, "category")}
            </span>
          </button>
        ),
        cell: (item) => {
      ...
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setItemSort((current) => toggleSort(current, "price"))
            }
            className={sortableHeaderClass(itemSort.key === "price")}
          >
            Giá{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(itemSort, "price")}
            </span>
          </button>
        ),
        cell: (item) => {
          ...
          return (
            <div>
              <p className="font-semibold">
                {formatVnd(
                  getItemEffectivePrice(item, category),
                  getItemCurrency(item, category),
                )}
              </p>
              <div className="mt-1 flex flex-wrap gap-1">
                <span
                  className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${hasOverride ? "bg-[var(--primary-fixed)] text-[var(--on-primary-fixed-variant)]" : "bg-[var(--surface-container-high)] text-[var(--on-surface-variant)]"}`}
                >
                  {hasOverride ? "Giá riêng" : "Theo nhóm"}
                </span>
                <span className="inline-flex rounded-full bg-[var(--surface-container-high)] px-2.5 py-1 text-xs font-bold text-[var(--on-surface-variant)]">
                  {formatQuantityRule(item)}
                </span>
              </div>
            </div>
          );
      ...
      },
      {
        ...
        header: (
          <button
            type="button"
            onClick={() =>
              setItemSort((current) => toggleSort(current, "status"))
            }
            className={sortableHeaderClass(itemSort.key === "status")}
          >
            Trạng thái{" "}
            <span className="text-[10px] normal-case tracking-normal">
              {sortIndicator(itemSort, "status")}
            </span>
          </button>
        ),
        cell: (item) => (
          <span
            className={`rounded-full px-2.5 py-1 text-xs font-bold ${serviceStatusTone(item.status)}`}
          >
  ...
  async function saveCategory(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      !/^\d+$/.test(categoryForm.defaultPrice) ||
    ...
    try {
      showLoading(
        categoryForm.id ? "Đang lưu nhóm dịch vụ" : "Đang tạo nhóm dịch vụ",
      );
      const priceChanged =
        categoryForm.defaultPrice !== categoryForm.originalDefaultPrice;
      const body = {
        ...
        ...(categoryForm.id && priceChanged
          ? { priceUpdateMode: categoryForm.priceUpdateMode }
          : {}),
        sortOrder,
      ...
      const saved = (
        await requestInternalApiEnvelope<HotelServiceCategory>(path, {
      ...
      setItems((current) =>
        syncItemsForCategoryUpdate(
          current,
          saved,
          categoryForm.priceUpdateMode,
        ),
      );
  ...
  async function refreshServiceCatalog() {
    ...
    const [categoriesPage, itemsPage] = await Promise.all([
      requestInternalApi<CatalogPage<HotelServiceCategory>>(
        `/api/owner/hotels/${encodedHotelId}/service-categories`,
        { method: "GET" },
      ),
      requestInternalApi<CatalogPage<HotelServiceItem>>(
        `/api/owner/hotels/${encodedHotelId}/service-items`,
        { method: "GET" },
      ),
    ]);
  ...
  async function saveItem(event: FormEvent<HTMLFormElement>) {
    ...
    const maxQuantity = itemForm.maxQuantity.trim()
      ? toPositiveInteger(itemForm.maxQuantity)
      : undefined;
    if (
      itemForm.quantityEnabled &&
    ...
    try {
      ...
      const body = {
        ...
        priceOverride: itemForm.priceOverride.trim()
          ? Number(itemForm.priceOverride)
          : null,
      ...
      const saved = (
        await requestInternalApiEnvelope<HotelServiceItem>(path, {
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-3 rounded-xl border border-[var(--outline-variant)] bg-white p-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex rounded-lg bg-[var(--surface-container-low)] p-1">
          <button
            ...
            className={`cursor-pointer rounded-md px-4 py-2 text-sm font-semibold transition-colors ${tab === "categories" ? "bg-white text-[var(--primary)] shadow-sm" : "text-[var(--on-surface-variant)] hover:text-[var(--primary)]"}`}
          >
            Nhóm dịch vụ
          </button>
          <button
            ...
            className={`cursor-pointer rounded-md px-4 py-2 text-sm font-semibold transition-colors ${tab === "items" ? "bg-white text-[var(--primary)] shadow-sm" : "text-[var(--on-surface-variant)] hover:text-[var(--primary)]"}`}
          >
            Dịch vụ
          </button>
        </div>
      ...
      {categoryForm ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4">
          ...
          >
            <h2 className="mb-5 text-2xl font-semibold text-[var(--primary)]">
              {categoryForm.id ? "Sửa nhóm dịch vụ" : "Tạo nhóm dịch vụ"}
            </h2>
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-2 md:col-span-2">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                />
              </label>
              <label className="space-y-2 md:col-span-2">
                <span className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--on-surface-variant)]">
                  ID group Telegram (tùy chọn)
                </span>
                <input
                  value={categoryForm.id_group}
                  onChange={(event) =>
                    setCategoryForm({
                      ...categoryForm,
                ...
                <span className="block text-xs text-[var(--on-surface-variant)]">
                  Chỉ nhập khi nhóm dịch vụ cần bắn thông báo vào group Telegram
                  riêng.
                </span>
              </label>
              <label className="space-y-2">
                ...
                <input
                  ...
                  inputMode="numeric"
                  autoComplete="off"
                  value={formatPriceInput(categoryForm.defaultPrice)}
                  onChange={(event) =>
                    setCategoryForm({
                      ...categoryForm,
                      defaultPrice: toRawPriceDigits(event.target.value),
                    })
                  }
                  onBeforeInput={(event) => {
              ...
              <label className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCategoryForm({
                      ...categoryForm,
                      status: event.target.value as HotelServiceStatus,
                    })
                  }
                  className="w-full cursor-pointer rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                >
              ...
              <label className="space-y-2 md:col-span-2">
                ...
                <textarea
                  ...
                  className="min-h-24 w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                />
              </label>
            </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {catalogLocales.map(({ key, label }) => (
                  <button
                    ...
                    aria-label={label}
                    onClick={() =>
                      setCategoryForm({ ...categoryForm, activeLocale: key })
                    }
                    className={`rounded-full px-3 py-1.5 text-xs font-semibold uppercase transition ${categoryForm.activeLocale === key ? "bg-[var(--primary)] text-[var(--on-primary)]" : "bg-[var(--surface-container-low)] text-[var(--on-surface-variant)] hover:text-[var(--primary)]"}`}
                  >
                    {key}
                  </button>
                ))}
              </div>
              {categoryForm.activeLocale === "vi" ? (
                <p className="mt-3 text-xs text-[var(--on-surface-variant)]">
                  Tiếng Việt dùng các trường Tên nhóm dịch vụ và Mô tả ở trên.
                </p>
              ) : (
                  ...
                  <input
                    value={
                      categoryForm.translations[
                        categoryForm.activeLocale as TranslationLocale
                      ].name
                    }
                    onChange={(event) =>
                      setCategoryForm({
                    ...
                    placeholder={`Tên (${categoryForm.activeLocale})`}
                    className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                  />
                  <textarea
                    value={
                    ...
                    placeholder={`Mô tả (${categoryForm.activeLocale})`}
                    className="min-h-20 w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)] md:col-span-2"
                  />
                </div>
              )}
            </section>
            {categoryForm.id ? (
              <fieldset className="mt-4 rounded-lg border border-[var(--outline-variant)] p-4">
                <legend className="px-1 text-sm font-semibold text-[var(--primary)]">
                  Phạm vi cập nhật giá
                </legend>
                <div className="mt-2 space-y-2">
                  <label className="flex cursor-pointer items-center gap-2 text-sm">
                    <input
                      ...
                      checked={categoryForm.priceUpdateMode === "CATEGORY_ONLY"}
                      onChange={() =>
                        setCategoryForm({
                          ...categoryForm,
                          priceUpdateMode: "CATEGORY_ONLY",
                    ...
                    />
                    Chỉ cập nhật giá mặc định nhóm
                  </label>
                  <label className="flex cursor-pointer items-center gap-2 text-sm">
                    <input
                      type="radio"
                      name="priceUpdateMode"
                      checked={
                        categoryForm.priceUpdateMode === "OVERRIDE_ALL_ITEMS"
                      }
                      onChange={selectOverrideAllItemsMode}
                    />
                    Đồng bộ giá cho toàn bộ dịch vụ trong nhóm
                  </label>
                </div>
              </fieldset>
            ) : null}
            <div className="mt-6 flex justify-end gap-2">
              <button
                ...
                className="cursor-pointer rounded-lg px-4 py-2 text-sm font-semibold transition-colors hover:bg-[var(--surface-container-low)]"
              >
                Hủy
              </button>
              <button
      ...
      {itemForm ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4">
          ...
          >
            <h2 className="mb-5 text-2xl font-semibold text-[var(--primary)]">
              {itemForm.id ? "Sửa dịch vụ" : "Tạo dịch vụ"}
            </h2>
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-2">
                <span className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--on-surface-variant)]">
                ...
                <input
                  ...
                  placeholder="VD: Bàn chải đánh răng"
                  className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                />
              </label>
              <label className="space-y-2">
              ...
              <label className="space-y-2">
                ...
                <select
                  ...
                  className="w-full cursor-pointer rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                >
                  {hotelServiceStatuses.map((status) => (
              ...
              <label className="space-y-2">
                ...
                <input
                  ...
                  inputMode="numeric"
                  autoComplete="off"
                  value={formatPriceInput(itemForm.priceOverride)}
                  onChange={(event) =>
                    setItemForm({
                      ...itemForm,
                      priceOverride: toRawPriceDigits(event.target.value),
                    })
                  }
                  onBeforeInput={(event) => {
                    if (event.data && /\D/.test(event.data))
              ...
              <p className="text-xs text-[var(--on-surface-variant)] md:col-span-2">
                Để trống giá riêng để sử dụng giá mặc định của nhóm dịch vụ.
              </p>
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-[var(--outline-variant)] p-3 md:col-span-2">
                <input
                  type="checkbox"
                  checked={itemForm.quantityEnabled}
                  onChange={(event) =>
                    setItemForm({
                      ...itemForm,
                ...
                <span>
                  <span className="block text-sm font-semibold text-[var(--primary)]">
                    Yêu cầu khách chọn số lượng
                  </span>
                  <span className="block text-xs text-[var(--on-surface-variant)]">
                    Bật khi dịch vụ cần số lượng như khăn, nước uống, bàn chải.
                  </span>
                </span>
              </label>
              <label className="space-y-2">
                ...
                <input
                  ...
                  min={1}
                  step={1}
                  disabled={!itemForm.quantityEnabled}
                  ...
                  placeholder="VD: 1"
                  className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)] disabled:bg-[var(--surface-container-low)] disabled:text-[var(--on-surface-variant)]"
                />
              </label>
              <label className="space-y-2">
                <span className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--on-surface-variant)]">
              ...
              <label className="space-y-2">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setItemForm({ ...itemForm, sortOrder: event.target.value })
                  }
                  placeholder="VD: 13"
                  className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                />
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {catalogLocales.map(({ key, label }) => (
                  <button
                    ...
                    aria-label={label}
                    onClick={() =>
                      setItemForm({ ...itemForm, activeLocale: key })
                    }
                    className={`rounded-full px-3 py-1.5 text-xs font-semibold uppercase transition ${itemForm.activeLocale === key ? "bg-[var(--primary)] text-[var(--on-primary)]" : "bg-[var(--surface-container-low)] text-[var(--on-surface-variant)] hover:text-[var(--primary)]"}`}
                  >
                    {key}
                  </button>
                ))}
              </div>
              {itemForm.activeLocale === "vi" ? (
                <p className="mt-3 text-xs text-[var(--on-surface-variant)]">
                  Tiếng Việt dùng các trường Tên dịch vụ và Mô tả ở trên.
                </p>
              ) : (
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  <input
                    value={
                      itemForm.translations[
                        itemForm.activeLocale as TranslationLocale
                      ].name
                    }
                    onChange={(event) =>
                      setItemForm({
                    ...
                    placeholder={`Tên (${itemForm.activeLocale})`}
                    className="w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)]"
                  />
                  <textarea
                    value={
                    ...
                    placeholder={`Mô tả (${itemForm.activeLocale})`}
                    className="min-h-20 w-full rounded-lg border border-[var(--outline-variant)] px-3 py-2 outline-none focus:border-[var(--primary)] md:col-span-2"
                  />
                </div>
              )}
