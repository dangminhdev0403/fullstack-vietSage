...
export default async function HotelServicesPage({ params }: ServicesPageProps) {
  ...
  if (
    !canUseHotelId(workspaceContext, hotelId) ||
    (!workspaceContext.permissions.includes("hotel.services.view") &&
      !workspaceContext.permissions.includes("hotel.services.manage"))
  ) {
    notFound();
  }
  ...
  const [categoriesPage, itemsPage] = await Promise.all([
    authorizedApi("list service categories", (accessToken) =>
      hotelOpsService.listServiceCategories(hotelId, {
        query: { page: 1, limit: 100 },
        accessToken,
      }),
    ),
    authorizedApi("list service items", (accessToken) =>
