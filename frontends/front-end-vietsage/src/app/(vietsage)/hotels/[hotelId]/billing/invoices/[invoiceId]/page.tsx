  ...
  return (
    <>
      ...
      <article className="invoice-a4 mx-auto w-full max-w-[850px] rounded-lg border border-slate-200 bg-white p-7 text-slate-950 shadow-sm print:border-0 print:p-0 print:shadow-none">
        ...
        <section className="py-5"><h3 className="text-lg font-black">Chi tiết chi phí</h3><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-xs"><thead className="bg-slate-100"><tr><th className="p-2">Nội dung</th><th className="p-2">Loại</th><th className="p-2 text-right">SL</th><th className="p-2 text-right">Đơn giá</th><th className="p-2 text-right">Tổng</th></tr></thead><tbody className="divide-y">{items.map((item) => <tr key={item.id}><td className="p-2 font-bold"><div>{item.name}</div>{item.description ? <div className="mt-0.5 text-[11px] font-normal text-slate-500">{item.description}</div> : null}</td><td className="p-2">{item.type}</td><td className="p-2 text-right">{item.quantity}</td><td className="p-2 text-right">{formatMoney(item.unitPrice, invoice.currency)}</td><td className="p-2 text-right font-bold">{formatMoney(item.total, invoice.currency)}</td></tr>)}</tbody></table></div></section>
