...
test("trangchu renders complete Vietnamese marketing copy without changing shared English defaults", () => {
  ...
  assert.match(shell, /Trợ lý số VietSage/);
  ...
  assert.match(shell, /Ẩm thực tại phòng/);
  ...
  assert.match(shell, /locale = "vi"/);
  assert.match(shell, /aria-label="Danh mục điều hướng trang chủ"/);
