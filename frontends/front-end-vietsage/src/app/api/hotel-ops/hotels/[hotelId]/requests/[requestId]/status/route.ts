...
export async function PATCH(request: Request, context: Params) {
  ...
  if (!parsed.success) return validationErrorResponse("Request status payload is invalid");
  try {
    const data = await executeHotelOpsBackendRequest("update hotel request status", (accessToken) =>
      hotelOpsService.updateRequestStatus(hotelId, requestId, parsed.data, accessToken),
    );
