...
export async function POST(_request: Request, { params }: Params) {
  ...
  try {
    const data = await executeHotelOpsBackendRequest("issue hotel marketplace service voucher", (token) =>
      localPartnersServerClient.issueVoucher(hotelId, orderId, token),
    );
