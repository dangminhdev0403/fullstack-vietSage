...
export async function POST(_request: Request, { params }: Params) {
  ...
  try {
    const data = await executeHotelOpsBackendRequest("acknowledge hotel marketplace order", (token) =>
      localPartnersServerClient.acknowledgeOrder(hotelId, orderId, token),
    );
