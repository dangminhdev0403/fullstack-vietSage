...
export async function GET(request: Request, context: Context) {
  ...
  try {
    const data = key === "categories"
      ? await guestOsService.listMarketplaceCategories<MarketplaceCategory[]>(token, locale)
      : key === "orders"
        ? await guestOsService.listMarketplaceOrders<MarketplaceOrder[]>(token, locale)
        : await guestOsService.listMarketplaceServices<MarketplaceServicesPage>(token, new URL(request.url).searchParams.get("categoryId") || undefined, locale);
