...
export async function handleKbttRequest(
  ...
  if (method === "PUT") {
    const parsed = kbttCredentialsSchema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) return validationErrorResponse("Vui lòng nhập tài khoản và mật khẩu hợp lệ.");
    credentials = parsed.data;
  }

  try {
    const result = await executeOwnerBackendRequest(`kbtt ${method}`, async (accessToken) => {
      ...
      return {
        ...connection,
        lastErrorCode: connection.lastErrorCode ? kbttErrorCode({ code: connection.lastErrorCode }) : null,
        lastErrorMessage: connection.lastErrorCode || connection.lastErrorMessage
    ...
    return NextResponse.json({ status: 200, error: null, message: "OK", data: result }, {
    ...
    return NextResponse.json({ status, message: "KBTT_ERROR", data: { code, detail } }, {
      status,
      headers: { "Cache-Control": "no-store" },
    });
  } finally {
