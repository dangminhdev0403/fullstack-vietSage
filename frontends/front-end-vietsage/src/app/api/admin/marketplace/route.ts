...
const updateTenant = z.object({ action: z.literal("updateTenant"), id: z.string().min(1), input: z.object({ displayName: z.string().trim().min(1).max(160).optional(), categoryId: z.string().min(1).optional(), googleSheetsUrl: z.string().trim().max(500).optional(), deliveryServiceFeeRate: z.number().min(0).max(100).nullable().optional(), status: z.string().trim().min(1).max(40).optional(), owner: z.object({ email: z.string().email().optional(), fullName: z.string().trim().min(2).max(120).optional(), password: z.string().min(8).max(128).optional() }).optional() }) });
