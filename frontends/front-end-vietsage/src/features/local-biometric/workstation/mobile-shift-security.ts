...
export async function limitedBytes(request: Request, max = 15 * 1024 * 1024): Promise<Uint8Array> {
  const length = request.headers.get("content-length");
  if (length !== null && (!/^\d+$/.test(length) || Number(length) > max)) throw new RangeError("BODY_LIMIT");
  const reader = request.body?.getReader();
  if (!reader) throw new Error("INVALID_BODY");
  ...
  try {
    for (;;) {
      ...
      if (size > max) { await reader.cancel(); throw new RangeError("BODY_LIMIT"); }
      chunks.push(value);
    }
    if (!size) throw new Error("INVALID_BODY");
