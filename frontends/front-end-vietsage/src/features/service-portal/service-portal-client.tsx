...
function nextStatus(order: MarketplaceOrder): string | null {
  ...
  if (order.status === "PREPARING") return order.serviceModeSnapshot === "DELIVERY_TO_HOTEL" ? "DELIVERING" : "READY";
  if (order.status === "READY" || order.status === "DELIVERING") return "COMPLETED";
  return null;
}
...
function Loaded({ value }: { value: ServicePortalData }) {
  ...
  const createService = (event: FormEvent<HTMLFormElement>) => {
    ...
    create.mutate({ categoryId: String(form.get("categoryId")), name: String(form.get("name")), unitPrice: Number(form.get("unitPrice")), imageUrls: [], mode: String(form.get("mode")), capacityAvailable: form.get("capacity") ? Number(form.get("capacity")) : null, waitingMinutes: Number(form.get("waitingMinutes")), status: "ACTIVE" });
  };
  return <main className="mx-auto max-w-6xl space-y-8 p-6"><header><p className="font-semibold text-emerald-700">SERVICE TENANT</p><h1 className="text-3xl font-bold">{value.profile.displayName}</h1></header>
    ...
    <section className="grid gap-5 lg:grid-cols-[360px_1fr]"><form onSubmit={createService} className="space-y-3 rounded-2xl border bg-white p-5"><h2 className="text-xl font-bold">Thêm dịch vụ</h2><select required name="categoryId" className={inputClass}><option value="">Chọn danh mục</option>{value.categories.map((item) => <option key={item.id} value={item.id}>{item.nameVi}</option>)}</select><input required name="name" placeholder="Tên dịch vụ" className={inputClass}/><input required min={0} type="number" name="unitPrice" placeholder="Giá VND" className={inputClass}/><input min={0} type="number" name="capacity" placeholder="Capacity; trống = không giới hạn" className={inputClass}/><input required min={0} type="number" name="waitingMinutes" placeholder="Thời gian chờ (phút)" className={inputClass}/><select name="mode" className={inputClass}><option value="CUSTOMER_AT_SERVICE">Khách đến địa điểm</option><option value="DELIVERY_TO_HOTEL">Giao đến khách sạn</option></select><button disabled={create.isPending} className="min-h-11 rounded-xl bg-emerald-700 px-5 font-semibold text-white">Tạo dịch vụ</button></form><div><h2 className="text-xl font-bold">Dịch vụ</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{value.services.map((item) => <article key={item.id} className="rounded-xl border bg-white p-4"><h3 className="font-bold">{item.name}</h3><p>{Number(item.unitPrice).toLocaleString("vi-VN")} VND · Chờ {item.waitingMinutes} phút</p><p className="text-sm">Capacity: {item.capacityAvailable ?? "Không giới hạn"} · {item.status}</p></article>)}</div></div></section>
