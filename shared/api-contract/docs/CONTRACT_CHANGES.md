...
## 2026-09-22 — Health readiness

- Added public `GET /health/ready`. Success confirms the application can query PostgreSQL; dependency
  failure returns `503`. Existing `GET /health` remains process-only liveness.
...
- GET never contacts KBTT. POST check is manual, returns HTTP 200 on success, refreshes near expiry (60 seconds), otherwise reauthenticates; a failed refresh falls back to encrypted credentials. Failure preserves credentials/configured=true, records safe AUTH_FAILED/lastCheckedAt, clears RAM token and rejects with HTTP 422 `KBTT_AUTH_FAILED`. No stored connection returns 404 `KBTT_NOT_CONFIGURED`; unavailable encryption/config/storage returns 503 `KBTT_UNAVAILABLE`. No cron, polling or guest submission is included.
- DELETE best-effort revokes, removes saved credentials/local session, returns the disconnected shape. Successful CONNECTED means the last manual authentication succeeded, not continuously monitored provider health. Runtime secret names and key-loss/rotation limits are documented in `docs/SECRETS.md`; controller OpenAPI annotations define the export source.
