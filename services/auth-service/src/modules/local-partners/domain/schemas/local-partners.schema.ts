...
const localPartnerBodySchema = z.object({
  ...
  name: z
    .string()
    .trim()
    .min(2, "Tên đối tác tối thiểu 2 ký tự")
    .max(160, "Tên đối tác tối đa 160 ký tự"),
  ...
  phone: z
    .string()
    .trim()
    .regex(/^[+\d][\d\s().-]{2,39}$/, "Số điện thoại không hợp lệ")
    .optional(),
  zaloUrl: z
    .string()
    .url()
    .refine((value) => /^https?:/.test(value), "URL phải dùng HTTP/HTTPS")
    .max(255)
    .optional(),
  websiteUrl: z
    .string()
...
export const createLocalPartnerBodySchema =
  localPartnerBodySchema.superRefine(requireCoordinatePair);
export const updateLocalPartnerBodySchema = localPartnerBodySchema
  .partial()
...
export const bookingRequestIdParamSchema = z
  .string()
  .min(1, "bookingRequestId không được để trống");
