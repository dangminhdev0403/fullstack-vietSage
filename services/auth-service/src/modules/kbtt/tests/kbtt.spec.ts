...
describe("KBTT Declaration Submission API 4/5", () => {
  ...
  it("handles business rejection by transitioning SENDING -> FAILED with sanitized provider code and message, allowing retry", async () => {
    ...
    expect(stored?.providerMessage).not.toContain("secret_token_xyz");
    const firstSnapshot = structuredClone(stored?.submittedPayloadJson);
    stored!.draftPayloadJson = { ...stored!.draftPayloadJson, hoTen: "MUTATED AFTER FAILURE" };
    ...
    expect(f.provider.submitDeclaration.mock.calls.at(-1)?.[1]).toEqual(firstSnapshot);
  });
