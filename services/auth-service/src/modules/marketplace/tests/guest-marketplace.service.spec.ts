  ...
  it("sorts mapped services by priority then distance and keeps unknown distance null", async () => {
    const prisma = {
      ...
      marketplaceService: {
        findMany: jest
          .fn()
          .mockResolvedValue([row("unknown", null), row("far", 1), row("near", 0.1)]),
      },
    };
    ...
    expect(prisma.marketplaceService.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          serviceTenant: expect.objectContaining({
            hotelServiceLinks: { some: { hotelId: "hotel-a", status: "ACTIVE" } },
          }),
        }),
      }),
    );
