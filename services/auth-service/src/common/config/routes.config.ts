...
export const PUBLIC_REGEX: RegExp[] = [
  /^\/guest\/(?:qr\/scan|session\/(?:me|close)|services|service-categories\/[^/]+\/services|requests|requests\/[^/]+\/cancel|messages|messages\/read|messages\/unread-summary|local-partners(?:\/categories|\/[^/]+)?|marketplace\/(?:categories|services(?:\/[^/]+)?|orders(?:\/[^/]+)?))$/,
